...
class mymodel(object):
    ...
    def build_model(self,args):    
        ...
        self.x_scale =   self.x /127.5-1 ## scale to range [-1 1]
        self.y1_scale=self.y1 /127.5 -1
        self.y2_scale=self.y2/127.5 -1
        self.y3_scale=self.y3 /127.5 -1
        self.y4_scale=self.y4 /127.5 -1
        self.y5_scale=self.y5 /127.5 -1
        self.net_g   =  SRGAN_delta(self.x_scale, is_train= True, reuse=False)
        ...
        self.test_input_scale = self.test_input/127.5-1
        self.test_output = SRGAN_delta(self.test_input_scale,is_train=False,reuse=True)
    ...
    def train(self, args):
        ...
        for iter_count in range(start_step,int(args.iteration_num)): 
            ...
            self.sess.run(self.global_step_add1)##### global step add 1
    ...
    def calculate_ReguTerm(self):
        ...
        pixel_grad13= (self.net_g[:, :-2, :-1,:]-self.net_g[:, 2:, 1: ,:])-(self.y1_scale[:, :-2, :-1,:]-self.y1_scale[:,2:,1: ,:])
        pixel_grad14= (self.net_g[:, 2: , 1: ,:]-self.net_g[:, :-2, :-1,:])-(self.y1_scale[:, 2: , 1: ,:]-self.y1_scale[:, :-2, :-1,:])
        pixel_grad15= (self.net_g[:, 2: , :-1,:]-self.net_g[:, :-2, 1: ,:])-(self.y1_scale[:, 2: , :-1,:]-self.y1_scale[:, :-2, 1: ,:])
        pixel_grad16= (self.net_g[:, :-2 , 1: ,:]-self.net_g[:, 2:, :-1,:])-(self.y1_scale[:, :-2 , 1: ,:]-self.y1_scale[:, 2:, :-1 ,:])
        pixel_grad17= (self.net_g[:, :-1, :-2,:]-self.net_g[:, 1:, 2: ,:])-(self.y1_scale[:, :-1, :-2,:]-self.y1_scale[:,1:,2: ,:])
        pixel_grad18= (self.net_g[:, 1: , 2: ,:]-self.net_g[:, :-1, :-2,:])-(self.y1_scale[:, 1: , 2: ,:]-self.y1_scale[:, :-1, :-2,:])
        pixel_grad19= (self.net_g[:, 1: , :-2,:]-self.net_g[:, :-1, 2: ,:])-(self.y1_scale[:, 1: , :-2,:]-self.y1_scale[:, :-1, 2: ,:])
        pixel_grad20= (self.net_g[:, :-1 , 2: ,:]-self.net_g[:, 1:, :-2,:])-(self.y1_scale[:, :-1 , 2: ,:]-self.y1_scale[:, 1:, :-2 ,:])
        pixel_grad21= (self.net_g[:, :-2, :-2,:]-self.net_g[:, 2:, 2: ,:])-(self.y1_scale[:, :-2, :-2,:]-self.y1_scale[:,2:,2: ,:])
        pixel_grad22= (self.net_g[:, 2: , 2: ,:]-self.net_g[:, :-2, :-2,:])-(self.y1_scale[:, 2: , 2: ,:]-self.y1_scale[:, :-2, :-2,:])
        pixel_grad23= (self.net_g[:, 2: , :-2,:]-self.net_g[:, :-2, 2: ,:])-(self.y1_scale[:, 2: , :-2,:]-self.y1_scale[:, :-2, 2: ,:])
        pixel_grad24= (self.net_g[:, :-2 , 2: ,:]-self.net_g[:, 2:, :-2,:])-(self.y1_scale[:, :-2 , 2: ,:]-self.y1_scale[:, 2:, :-2 ,:])
        ...
        ReguTerm1 = tf.reduce_mean(tf.abs(pixel_grad1)) \
                      ...
                      + tf.reduce_mean(tf.abs(pixel_grad13)) \
                      + tf.reduce_mean(tf.abs(pixel_grad14)) \
                      + tf.reduce_mean(tf.abs(pixel_grad15)) \
                      + tf.reduce_mean(tf.abs(pixel_grad16)) \
                      + tf.reduce_mean(tf.abs(pixel_grad17)) \
                      + tf.reduce_mean(tf.abs(pixel_grad18)) \
                      + tf.reduce_mean(tf.abs(pixel_grad19)) \
                      + tf.reduce_mean(tf.abs(pixel_grad20)) \
                      + tf.reduce_mean(tf.abs(pixel_grad21)) \
                      + tf.reduce_mean(tf.abs(pixel_grad22)) \
                      + tf.reduce_mean(tf.abs(pixel_grad23)) \
                      + tf.reduce_mean(tf.abs(pixel_grad24))
        ...
        pixel_grad13= (self.net_g[:, :-2, :-1,:]-self.net_g[:, 2:, 1: ,:])-(self.y2_scale[:, :-2, :-1,:]-self.y2_scale[:,2:,1: ,:])
        pixel_grad14= (self.net_g[:, 2: , 1: ,:]-self.net_g[:, :-2, :-1,:])-(self.y2_scale[:, 2: , 1: ,:]-self.y2_scale[:, :-2, :-1,:])
        pixel_grad15= (self.net_g[:, 2: , :-1,:]-self.net_g[:, :-2, 1: ,:])-(self.y2_scale[:, 2: , :-1,:]-self.y2_scale[:, :-2, 1: ,:])
        pixel_grad16= (self.net_g[:, :-2 , 1: ,:]-self.net_g[:, 2:, :-1,:])-(self.y2_scale[:, :-2 , 1: ,:]-self.y2_scale[:, 2:, :-1 ,:])
        pixel_grad17= (self.net_g[:, :-1, :-2,:]-self.net_g[:, 1:, 2: ,:])-(self.y2_scale[:, :-1, :-2,:]-self.y2_scale[:,1:,2: ,:])
        pixel_grad18= (self.net_g[:, 1: , 2: ,:]-self.net_g[:, :-1, :-2,:])-(self.y2_scale[:, 1: , 2: ,:]-self.y2_scale[:, :-1, :-2,:])
        pixel_grad19= (self.net_g[:, 1: , :-2,:]-self.net_g[:, :-1, 2: ,:])-(self.y2_scale[:, 1: , :-2,:]-self.y2_scale[:, :-1, 2: ,:])
        pixel_grad20= (self.net_g[:, :-1 , 2: ,:]-self.net_g[:, 1:, :-2,:])-(self.y2_scale[:, :-1 , 2: ,:]-self.y2_scale[:, 1:, :-2 ,:])
        pixel_grad21= (self.net_g[:, :-2, :-2,:]-self.net_g[:, 2:, 2: ,:])-(self.y2_scale[:, :-2, :-2,:]-self.y2_scale[:,2:,2: ,:])
        pixel_grad22= (self.net_g[:, 2: , 2: ,:]-self.net_g[:, :-2, :-2,:])-(self.y2_scale[:, 2: , 2: ,:]-self.y2_scale[:, :-2, :-2,:])
        pixel_grad23= (self.net_g[:, 2: , :-2,:]-self.net_g[:, :-2, 2: ,:])-(self.y2_scale[:, 2: , :-2,:]-self.y2_scale[:, :-2, 2: ,:])
        pixel_grad24= (self.net_g[:, :-2 , 2: ,:]-self.net_g[:, 2:, :-2,:])-(self.y2_scale[:, :-2 , 2: ,:]-self.y2_scale[:, 2:, :-2 ,:])
        ...
        pixel_grad13= (self.net_g[:, :-2, :-1,:]-self.net_g[:, 2:, 1: ,:])-(self.y3_scale[:, :-2, :-1,:]-self.y3_scale[:,2:,1: ,:])
        pixel_grad14= (self.net_g[:, 2: , 1: ,:]-self.net_g[:, :-2, :-1,:])-(self.y3_scale[:, 2: , 1: ,:]-self.y3_scale[:, :-2, :-1,:])
        pixel_grad15= (self.net_g[:, 2: , :-1,:]-self.net_g[:, :-2, 1: ,:])-(self.y3_scale[:, 2: , :-1,:]-self.y3_scale[:, :-2, 1: ,:])
        pixel_grad16= (self.net_g[:, :-2 , 1: ,:]-self.net_g[:, 2:, :-1,:])-(self.y3_scale[:, :-2 , 1: ,:]-self.y3_scale[:, 2:, :-1 ,:])
        pixel_grad17= (self.net_g[:, :-1, :-2,:]-self.net_g[:, 1:, 2: ,:])-(self.y3_scale[:, :-1, :-2,:]-self.y3_scale[:,1:,2: ,:])
        pixel_grad18= (self.net_g[:, 1: , 2: ,:]-self.net_g[:, :-1, :-2,:])-(self.y3_scale[:, 1: , 2: ,:]-self.y3_scale[:, :-1, :-2,:])
        pixel_grad19= (self.net_g[:, 1: , :-2,:]-self.net_g[:, :-1, 2: ,:])-(self.y3_scale[:, 1: , :-2,:]-self.y3_scale[:, :-1, 2: ,:])
        pixel_grad20= (self.net_g[:, :-1 , 2: ,:]-self.net_g[:, 1:, :-2,:])-(self.y3_scale[:, :-1 , 2: ,:]-self.y3_scale[:, 1:, :-2 ,:])
        pixel_grad21= (self.net_g[:, :-2, :-2,:]-self.net_g[:, 2:, 2: ,:])-(self.y3_scale[:, :-2, :-2,:]-self.y3_scale[:,2:,2: ,:])
        pixel_grad22= (self.net_g[:, 2: , 2: ,:]-self.net_g[:, :-2, :-2,:])-(self.y3_scale[:, 2: , 2: ,:]-self.y3_scale[:, :-2, :-2,:])
        pixel_grad23= (self.net_g[:, 2: , :-2,:]-self.net_g[:, :-2, 2: ,:])-(self.y3_scale[:, 2: , :-2,:]-self.y3_scale[:, :-2, 2: ,:])
        pixel_grad24= (self.net_g[:, :-2 , 2: ,:]-self.net_g[:, 2:, :-2,:])-(self.y3_scale[:, :-2 , 2: ,:]-self.y3_scale[:, 2:, :-2 ,:])
        ...
        pixel_grad13= (self.net_g[:, :-2, :-1,:]-self.net_g[:, 2:, 1: ,:])-(self.y4_scale[:, :-2, :-1,:]-self.y4_scale[:,2:,1: ,:])
        pixel_grad14= (self.net_g[:, 2: , 1: ,:]-self.net_g[:, :-2, :-1,:])-(self.y4_scale[:, 2: , 1: ,:]-self.y4_scale[:, :-2, :-1,:])
        pixel_grad15= (self.net_g[:, 2: , :-1,:]-self.net_g[:, :-2, 1: ,:])-(self.y4_scale[:, 2: , :-1,:]-self.y4_scale[:, :-2, 1: ,:])
        pixel_grad16= (self.net_g[:, :-2 , 1: ,:]-self.net_g[:, 2:, :-1,:])-(self.y4_scale[:, :-2 , 1: ,:]-self.y4_scale[:, 2:, :-1 ,:])
        pixel_grad17= (self.net_g[:, :-1, :-2,:]-self.net_g[:, 1:, 2: ,:])-(self.y4_scale[:, :-1, :-2,:]-self.y4_scale[:,1:,2: ,:])
        pixel_grad18= (self.net_g[:, 1: , 2: ,:]-self.net_g[:, :-1, :-2,:])-(self.y4_scale[:, 1: , 2: ,:]-self.y4_scale[:, :-1, :-2,:])
        pixel_grad19= (self.net_g[:, 1: , :-2,:]-self.net_g[:, :-1, 2: ,:])-(self.y4_scale[:, 1: , :-2,:]-self.y4_scale[:, :-1, 2: ,:])
        pixel_grad20= (self.net_g[:, :-1 , 2: ,:]-self.net_g[:, 1:, :-2,:])-(self.y4_scale[:, :-1 , 2: ,:]-self.y4_scale[:, 1:, :-2 ,:])
        pixel_grad21= (self.net_g[:, :-2, :-2,:]-self.net_g[:, 2:, 2: ,:])-(self.y4_scale[:, :-2, :-2,:]-self.y4_scale[:,2:,2: ,:])
        pixel_grad22= (self.net_g[:, 2: , 2: ,:]-self.net_g[:, :-2, :-2,:])-(self.y4_scale[:, 2: , 2: ,:]-self.y4_scale[:, :-2, :-2,:])
        pixel_grad23= (self.net_g[:, 2: , :-2,:]-self.net_g[:, :-2, 2: ,:])-(self.y4_scale[:, 2: , :-2,:]-self.y4_scale[:, :-2, 2: ,:])
        pixel_grad24= (self.net_g[:, :-2 , 2: ,:]-self.net_g[:, 2:, :-2,:])-(self.y4_scale[:, :-2 , 2: ,:]-self.y4_scale[:, 2:, :-2 ,:])
        ...
        pixel_grad13= (self.net_g[:, :-2, :-1,:]-self.net_g[:, 2:, 1: ,:])-(self.y5_scale[:, :-2, :-1,:]-self.y5_scale[:,2:,1: ,:])
        pixel_grad14= (self.net_g[:, 2: , 1: ,:]-self.net_g[:, :-2, :-1,:])-(self.y5_scale[:, 2: , 1: ,:]-self.y5_scale[:, :-2, :-1,:])
        pixel_grad15= (self.net_g[:, 2: , :-1,:]-self.net_g[:, :-2, 1: ,:])-(self.y5_scale[:, 2: , :-1,:]-self.y5_scale[:, :-2, 1: ,:])
        pixel_grad16= (self.net_g[:, :-2 , 1: ,:]-self.net_g[:, 2:, :-1,:])-(self.y5_scale[:, :-2 , 1: ,:]-self.y5_scale[:, 2:, :-1 ,:])
        pixel_grad17= (self.net_g[:, :-1, :-2,:]-self.net_g[:, 1:, 2: ,:])-(self.y5_scale[:, :-1, :-2,:]-self.y5_scale[:,1:,2: ,:])
        pixel_grad18= (self.net_g[:, 1: , 2: ,:]-self.net_g[:, :-1, :-2,:])-(self.y5_scale[:, 1: , 2: ,:]-self.y5_scale[:, :-1, :-2,:])
        pixel_grad19= (self.net_g[:, 1: , :-2,:]-self.net_g[:, :-1, 2: ,:])-(self.y5_scale[:, 1: , :-2,:]-self.y5_scale[:, :-1, 2: ,:])
        pixel_grad20= (self.net_g[:, :-1 , 2: ,:]-self.net_g[:, 1:, :-2,:])-(self.y5_scale[:, :-1 , 2: ,:]-self.y5_scale[:, 1:, :-2 ,:])
        pixel_grad21= (self.net_g[:, :-2, :-2,:]-self.net_g[:, 2:, 2: ,:])-(self.y5_scale[:, :-2, :-2,:]-self.y5_scale[:,2:,2: ,:])
        pixel_grad22= (self.net_g[:, 2: , 2: ,:]-self.net_g[:, :-2, :-2,:])-(self.y5_scale[:, 2: , 2: ,:]-self.y5_scale[:, :-2, :-2,:])
        pixel_grad23= (self.net_g[:, 2: , :-2,:]-self.net_g[:, :-2, 2: ,:])-(self.y5_scale[:, 2: , :-2,:]-self.y5_scale[:, :-2, 2: ,:])
        pixel_grad24= (self.net_g[:, :-2 , 2: ,:]-self.net_g[:, 2:, :-2,:])-(self.y5_scale[:, :-2 , 2: ,:]-self.y5_scale[:, 2:, :-2 ,:])
